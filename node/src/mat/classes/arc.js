...
class Arc {
    ...
    static closestPointOnArc(p, arc) {
        if (arc.circle !== null) {
            ...
            let arco = new Arc(new circle_1.Circle([0, 0], arc.circle.radius), arc.sinAngle1, arc.cosAngle1, arc.sinAngle2, arc.cosAngle2, translate(arc.startpoint), translate(arc.endpoint));
            let pp = translate(p);
            let l = Vector.len(pp);
            let sin_pp = -pp[1] / l;
            let cos_pp = pp[0] / l;
            if (geometry_1.Geometry.isAngleBetween(sin_pp, cos_pp, arco.sinAngle1, arco.cosAngle1, arco.sinAngle2, arco.cosAngle2)) {
        ...
        let res = {
            p: geometry_1.Geometry.lineLineIntersection([p, l1p2], [asp, aep]),
            position: 0,
        };
        return res;
    }
