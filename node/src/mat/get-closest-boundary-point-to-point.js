...
const closest_squared_distance_to_totated_rect_1 = require("./geometry/closest-squared-distance-to-totated-rect");
...
function getClosestBoundaryPointToPoint(bezierPieces_, point, touchedBezierNode, t) {
    ...
    for (let bezierPiece of bezierPieces) {
        let p = closestPointOnBezier(bezierPiece.curve, point, bezierPiece.ts, touchedBezierNode, t);
        let d = p === undefined
            ? Number.POSITIVE_INFINITY
            : Vector.distanceBetween(p.p, point);
        if (d < bestDistance) {
            pos = new point_on_shape_1.PointOnShape(bezierPiece.curve, p.t);
            bestDistance = d;
        }
    }
    return pos;
}
exports.getClosestBoundaryPointToPoint = getClosestBoundaryPointToPoint;
...
function cullByTightBoundingBox(bezierPieces, p, bestSquaredDistance) {
    ...
    for (let bezierPiece of bezierPieces) {
        ...
        let d = closest_squared_distance_to_totated_rect_1.closestSquaredDistanceToRotatedRect(tightBoundingBox, p);
        if (d <= bestSquaredDistance) {
            candidateBezierPieces.push(bezierPiece);
        }
    }
...
function closestPointOnBezier(bezierNode, p, tRange, touchedBezierNode, t) {
    ...
    let closestPs = roots.map(function (root) {
        return { p: Bezier3.evaluate(ps)(root), t: root };
    });
