...
function arcFrom3Points(ps) {
    ...
    if (isAngleBetween(sin_midangle, cos_midangle, sinθ1, cosθ1, sinθ2, cosθ2)) {
        arc = new arc_1.Arc(new circle_1.Circle(circleCenter, radius), sinθ1, cosθ1, sinθ2, cosθ2, ps[0], ps[2]);
    }
    else {
        arc = new arc_1.Arc(new circle_1.Circle(circleCenter, radius), sinθ2, cosθ2, sinθ1, cosθ1, ps[2], ps[0]);
    }
    return arc;
}
/**
...
function getLineBezierIntersectionPoints(line, ps, tRange) {
    ...
    let newPs = Vector.translateThenRotatePs(t, sinθ, cosθ, ps);
    let roots = flo_poly_1.default.allRoots(Bezier3.getY(newPs), 0, 1);
    return roots.map(t => ({ p: Bezier3.evaluate(ps)(t), t }));
}
const Geometry = {
