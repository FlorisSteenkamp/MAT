...
const copy_mat_1 = require("../functions/copy-mat");
//import { getMatNodesAsHash } from '../functions/get-nodes-as-hash';
...
function toScaleAxis(mat_, s) {
    ...
    let mat = copy_mat_1.copyMat(mat_);
    ...
    //let nodeHash = getMatNodesAsHash(mat);
    ...
    for (let key in nodeHash) {
        ...
        let matCircle = nodeHash[key] /*.matCircle*/;
    ...
    let sat = new mat_tree_1.MatTree(biggestCircle.matNode, biggestCircle);
    let cullHash = {};
...
function addToTree(s, tree, coordinate, limits, 
    //node: MatNode, 
    node, key, depth) {
...
function cullem(node, key, extraParams) {
    ...
    for (let key in cullNodes) {
        if (!cullHash[key]) {
            cullHash[key] = node /*.matCircle*/;
        }
    }
}
function traverseSpacialTree(tree, 
    ...
    function helper(tree) {
        ...
        if (tree.size) {
            tree.forEach(function (node, key) {
                f(node /*.matNode*/, key, extraParams);
            });
            return; // Leaf reached 
        }
        if (tree[5]) {
...
function cullTheNode(cullNode) {
    ...
    //let indx = priorNode.branches.indexOf(satNode);
    if (indx >= 0) {
