...
//import { getNeighbouringPoints } from '../find-mat/get-neighboring-points';
/**
 ...
 * placed
 */
function add2Prong(cpGraphs, circle, posSource, posAntipode, holeClosing) {
    //------------------------------------
    //------------ ANTIPODE --------------
    //------------------------------------
    ...
    let loopAntipode = posAntipode.bezierNode.loop;
    let cpGraphAntipode = cpGraphs.get(loopAntipode);
    let deltaAntipode = cp_graph_1.CpGraph.getNeighbouringPoints(cpGraphAntipode, posAntipode, orderAntipode, 0);
    ...
    let newCpAntipode = cpGraphAntipode.insert(cpAntipode, deltaAntipode[0]);
    let loopSource = posSource.bezierNode.loop;
    let cpGraphSource = cpGraphs.get(loopSource);
    //let deltaSource = getNeighbouringPoints(cpGraphSource, posSource, orderSource, 0);
    let deltaSource = cp_graph_1.CpGraph.getNeighbouringPoints(cpGraphSource, posSource, orderSource, 0);
    let newCpSource = cpGraphSource.insert(cpSource, deltaSource[0]);
    //let vertex = Vertex.create(circle, [newCpSource, newCpAntipode]);
    //newCpSource  .cp.circle = circle;
    //newCpAntipode.cp.circle = circle;
    ...
    if (holeClosing) {
        ...
        let posB2 = posA1;
        let cpB2 = new contact_point_1.ContactPoint(posB2, circle, cpAntipode.order, 1);
        let newCpB2Node = cpGraphAntipode.insert(cpB2, newCpAntipode);
        let posA2 = posSource;
        let posB1 = posA2;
        let cpB1 = new contact_point_1.ContactPoint(posB1, circle, cpSource.order, -1);
        let newCpB1Node = cpGraphSource.insert(cpB1, newCpSource.prev);
