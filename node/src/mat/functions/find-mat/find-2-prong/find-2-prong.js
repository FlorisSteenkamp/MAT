...
const get_boundary_beziers_1 = require("../../find-mat/get-boundary-beziers");
 ...
 */
function find2Prong(loops, cpGraphs, y, holeClosing, k) {
    ...
    if (holeClosing) {
        ...
        let order = y.type === mat_constants_1.MAT_CONSTANTS.pointType.dull
            ...
            : 0;
        let loop = loops[k];
        let ps = cp_graph_1.CpGraph.getNeighbouringPoints(cpGraphs.get(loop), y, order, 0);
        δ = [ps[0], ps[0]];
        if (!ps[0]) {
            bezierPieces = get_boundary_beziers_1.getBoundaryBeziers(loop);
        }
        else {
