...
function find2Prong(shape, y, holeClosing) {
    ...
    if (holeClosing) {
        ...
        for (let k2 = 0; k2 < k; k2++) {
            let pieces = shape_1.Shape.getBoundaryBeziers(shape, k2);
        ...
        let ps = shape_1.Shape.getNeighbouringPoints(shape, y);
        δ = [ps[0], ps[0]];
        if (!ps[0]) {
            bezierPieces = shape_1.Shape.getBoundaryBeziers(shape, k);
        }
        else {
            bezierPieces = shape_1.Shape.getBoundaryPieceBeziers(δ);
        }
    }
    ...
    let squaredError;
    let i = 0;
    //_debug_.fs.draw.circle(oCircle, ' green thin10 nofill')
    do {
        i++;
    ...
    point_on_shape_1.PointOnShape.setPointOrder(shape, circle, y);
    point_on_shape_1.PointOnShape.setPointOrder(shape, circle, z);
    if (typeof _debug_ !== 'undefined') {
...
function add1Prong(shape, pos) {
    if (pos.type === mat_constants_1.MAT_CONSTANTS.pointType.dull) {
        ...
        if (typeof _debug_ !== 'undefined') {
            ...
            if (!Number.isNaN(oCircle.center[0])) {
                ...
                /*
                _debug_.generated.svg.oneProngsAtDullCorner.all.push(
                    _debug_.fs.drawElem.draw1ProngAtDullCorner(pos, _debug_.config.toDraw.oneProngsAtDullCorner.all)
                );
                */
            }
        }
    ...
    */
    ...
    let newCp = shape.contactPointsPerLoop[k].insert(
            cp,
            delta[0]
    );
    */
    let vertex = vertex_1.Vertex.create2(point_on_shape_1.PointOnShape.getOsculatingCircle(pos), shape, [pos]);
    /*
    ...
    */
    /*
    newCp.prevOnCircle = newCp;  // Trivial loop
    newCp.nextOnCircle = newCp;  // ...
    */
    if (typeof _debug_ !== 'undefined') {
        ...
        /*
        _debug_.generated.svg.oneProngs.all.push(
            _debug_.fs.drawElem.draw1Prong(pos, _debug_.config.toDraw.oneProngs.all)
        );
        */
    }
    return;
...
function cullBezierPieces(bezierPieces, p, rSquared) {
    ...
    for (let bezierPiece of bezierPieces) {
        ...
        let bd = geometry_1.Geometry.getClosestSquareDistanceToRect(rect, p);
