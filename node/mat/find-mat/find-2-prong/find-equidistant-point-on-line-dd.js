import { twoDiff, twoSum, twoProduct, ddDivDd, ddAddDd, ddDiffDd, ddMultDd, ddMultBy2, ddMultDouble1 } from "double-double";
const { abs } = Math;
const td = twoDiff;
const ts = twoSum;
const tp = twoProduct;
...
function findEquidistantPointOnLineDd(x, y, z) {
    ...
    const c = ddAddDd((ddDiffDd(ddAddDd(tp(y1, y1), tp(y2, y2)), ddAddDd(tp(z1, z1), tp(z2, z2)))), ddMultBy2(ddMultDd(b, (td(z2, y2)))));
