...
const copy_mat_1 = require("./copy-mat");
const get_nodes_as_hash_1 = require("./get-nodes-as-hash");
...
function toScaleAxis(mat_, s) {
    ...
    let mat = copy_mat_1.default(mat_);
    ...
    let nodeHash = get_nodes_as_hash_1.default(mat);
    ...
    let sat = new mat_tree_1.default(biggestNode);
    let cullHash = {};
...
function getCullNodes(s, tree, testNode) {
    let c1 = circle_1.default.scale(testNode.matCircle.circle, s);
    let cullNodes = {};
    let limits = [[0, width], [0, height]];
    ...
    function cullBranch5(tree) {
        ...
        branch.forEach(function (node, key) {
            let c2 = circle_1.default.scale(node.matCircle.circle, s);
...
function cullIt(cullHash, cullNodes, satNode, priorNode) {
    let key = point_on_shape_1.default.makeSimpleKey(satNode.matCircle.circle.center);
