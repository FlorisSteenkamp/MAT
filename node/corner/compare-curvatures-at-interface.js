...
function compareCurvaturesAtInterface(psI, psO) {
    ...
    const a = eDiff(eMult(dxI, ddyI), eMult(dyI, ddxI));
    const c = eDiff(eMult(dxO, ddyO), eMult(dyO, ddxO));
    const signA = eSign(a);
    const signC = eSign(c);
    ...
    const b = fastExpansionSum(eMult(dxO, dxO), eMult(dyO, dyO));
    const d = fastExpansionSum(eMult(dxI, dxI), eMult(dyI, dyI));
    const b2 = eMult(b, b);
    const b3 = eMult(b2, b);
    const d2 = eMult(d, d);
    const d3 = eMult(d2, d);
    ...
    const e = eDiff(eMult(dxI, dddyI), eMult(dyI, dddxI));
    const f = fastExpansionSum(eMult(dxI, ddxI), eMult(dyI, ddyI));
    const g = eDiff(eMult(dxO, dddyO), eMult(dyO, dddxO));
    const h = fastExpansionSum(eMult(dxO, ddxO), eMult(dyO, ddyO));
    // (de - 3af)²b⁵ > (bg - 3ch)²d⁵
    // i²b⁵ > j²d⁵
    const i = eDiff(eMult(d, e), scaleExpansion(eMult(a, f), 3));
    const j = eDiff(eMult(b, g), scaleExpansion(eMult(c, h), 3));
    const signI = eSign(i);
    const signJ = eSign(j);
    if (signI !== signJ) {
        return signI - signJ;
