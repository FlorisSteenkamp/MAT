...
import { areBeziersInSameKFamily, getBoundingBox, isHorizontalLine, isVerticalLine, isLine } from 'flo-bezier3';
...
function inSameKFamliyAndMaybeIntersecting(ps1: number[][], ps2: number[][]) {
    ...
    if (isHorizontalLine(ps1) && isHorizontalLine(ps2)) {
        if (ps1[0][1] !== ps2[0][1]) { return false; }
    ...
    } else if (isVerticalLine(ps1) && isVerticalLine(ps2)) {
        if (ps1[0][0] !== ps2[0][0]) { return false; }
    ...
    return (
        areBoxesIntersectingOpen(bbPs1, bbPs2) &&
        areBeziersInSameKFamily(ps1, ps2)
    );
}


export { inSameKFamliyAndMaybeIntersecting }
