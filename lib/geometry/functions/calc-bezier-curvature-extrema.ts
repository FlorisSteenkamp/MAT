 ...
 */
function calcBezierCurvatureExtremaBrackets(ps: number[][]): number[][] {
 ...
 * @private
 */ 
function clipBoundary(bound: number[]): number[] {
	let b0 = bound[0];
	let b1 = bound[1];
 ...
 * @private
 */
function deReParameterize(lambda: number, mu: number, a_: number) {
	return (sigma: number) => (sigma - lambda) * (mu / a_);	
}
 ...
 * @private
 */
function deReParameterizeBoundary(lambda: number, mu: number, a_: number) {
	return (boundary: number[]) => boundary.map(deReParameterize(lambda, mu, a_));	
};
